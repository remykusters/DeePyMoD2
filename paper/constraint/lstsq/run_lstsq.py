...
for run_idx in np.arange(n_runs):
    ...
    sparsity_scheduler = TrainTestPeriodic(periodicity=25, patience=10000)
    optimizer = torch.optim.Adam(model.parameters(), betas=(0.99, 0.999), amsgrad=True, lr=2e-3) # Defining optimizer
    
    train_split_full(model, X, y, optimizer, sparsity_scheduler, split=0.8, log_dir=f'data/lstsq_run_{run_idx}/', write_iterations=25, max_iterations=5000, delta=0.000, patience=200) # Running
